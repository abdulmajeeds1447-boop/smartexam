...
export const TeacherDashboard: React.FC = () => {
  const { 
    ...
    markAttendance, submitEnvelope, logout 
  } = useApp();
      ...
      <div className="min-h-screen bg-[#050b14] text-white flex flex-col font-sans" dir="rtl">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-10">
            ...
            <div className="w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl border border-white/10">
                <Scanner onScanSuccess={() => setIsScanningMode(false)} />
            </div>
        </div>

        <footer className="py-8 text-center">
            <button onClick={logout} className="text-red-400 flex items-center gap-2 mx-auto font-bold text-sm bg-red-400/5 px-6 py-2 rounded-full border border-red-400/20">
    ...
    <div className="min-h-screen bg-slate-50 pb-36 font-sans text-right" dir="rtl">
      <div className="bg-[#050b14] text-white p-6 rounded-b-[2.5rem] shadow-2xl sticky top-0 z-30 border-b-4 border-blue-600">
        <div className="flex justify-between items-start mb-6">
            <div className="space-y-1">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <button onClick={() => setIsScanningMode(true)} className="flex items-center gap-1.5 bg-white/10 px-3 py-1 rounded-lg text-[10px] font-bold text-blue-300 hover:bg-white/20 transition-all border border-white/5">
        ...
        <div className="relative group">
            <input type="text" placeholder="ابحث عن طالب..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-12 text-sm focus:bg-white focus:text-slate-900 outline-none transition-all placeholder-white/30 font-bold shadow-inner" />
            ...
            return (
                <div key={gradeTitle} className="space-y-4 animate-fade-in">
                    <h3 className="font-black text-slate-800 flex items-center gap-3 border-r-4 border-blue-600 pr-4 py-1">
                        <Layers size={22} className="text-blue-600" /> {gradeTitle} <span className="text-[10px] font-bold text-slate-500 bg-blue-100/50 px-3 py-1 rounded-full">{students.length} طالب</span>
                    </h3>
                    <div className="grid grid-cols-1 gap-3">
                        {displayedStudents.map(student => {
                            ...
                            return (
                                <div key={student.id} className={`bg-white rounded-[2rem] p-5 shadow-sm border-2 transition-all flex items-center justify-between ${isPresent ? 'border-green-100' : isAbsent ? 'border-red-100' : 'border-slate-100'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-2xl text-slate-300 border border-slate-100 overflow-hidden shadow-inner">{student.name.charAt(0)}</div>
                                        <div><h4 className="font-black text-slate-800 text-base leading-tight mb-1">{student.name}</h4><span className="text-[10px] bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg font-bold shadow-sm">مقعد: {student.seatNumber}</span></div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => markAttendance(activeExam.id, student.id, AttendanceStatus.PRESENT)} className={`p-4 rounded-2xl transition-all ${isPresent ? 'bg-green-500 text-white' : 'bg-slate-50 text-slate-400'}`}><UserCheck size={28} /></button>
                                        <button onClick={() => markAttendance(activeExam.id, student.id, AttendanceStatus.ABSENT)} className={`p-4 rounded-2xl transition-all ${isAbsent ? 'bg-red-500 text-white' : 'bg-slate-50 text-slate-400'}`}><UserX size={28} /></button>
                                    </div>
                                </div>
                            );
                        })}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-2xl border-t border-slate-200 z-40 pb-safe">
          <button onClick={() => { if(window.confirm('هل أنت متأكد؟')) submitEnvelope(activeExam.id); }} className="w-full bg-[#050b14] text-white py-5 rounded-3xl font-black text-xl flex items-center justify-center gap-3 shadow-2xl active:scale-95 transition-all">
              <Send size={24} className="text-blue-400" /> إنهاء الرصد وتسليم المظروف
          </button>
      </div>
    </div>
  );
...
const FilterTab = ({ label, count, active, onClick, color = "bg-blue-600" }: any) => (
    <button onClick={onClick} className={`flex items-center gap-2.5 px-5 py-3 rounded-2xl border-2 transition-all whitespace-nowrap font-black text-xs ${active ? 'bg-white border-blue-600 shadow-lg text-slate-800 scale-105' : 'bg-transparent border-transparent text-slate-400'}`}>
        <span className={`w-2 h-2 rounded-full ${active ? color : 'bg-slate-300'}`}></span> {label} <span className={`px-2 py-0.5 rounded-lg text-[10px] ${active ? 'bg-slate-100 text-blue-800' : 'bg-slate-200/50 text-slate-400'}`}>{count}</span>
    </button>
);
