...
import { Scanner } from './Scanner'; // استيراد مكون الماسح الاحترافي
...
export const TeacherDashboard: React.FC = () => {
  ...
  // 1. تحديد اللجنة النشطة لهذا المعلم (بناءً على المسح وتوزيع الكنترول)
  ...
  // تحديث المعرف النشط تلقائياً لضمان استقرار الواجهة
  useEffect(() => {
  ...
  // 2. المحرك الذكي لترتيب المراحل (أول -> ثاني -> ثالث)
  const sortedGroupedStudents = useMemo(() => {
    if (!activeExam) return {};
    
    const groups: { [key: string]: Student[] } = {};
    
    // فلترة الطلاب حسب البحث (اسم أو رقم جلوس)
    const filtered = activeExam.students.filter(s => 
        s.name.includes(searchTerm) || s.seatNumber.includes(searchTerm)
    );

    // تجميع الطلاب حسب المرحلة الدراسية
    ...
    // دالة الأوزان لضمان الترتيب التصاعدي الصحيح
    const getWeight = (s: string) => {
        if (s.includes('أول') || s.includes('1')) return 1;
  ...
  // إغلاق وضع المسح عند النجاح
  const onScanSuccess = () => {
    setIsScanningMode(false);
  };

  // --- الواجهة 1: انتظار مسح باركود المظروف ---
      ...
      <div className="min-h-screen bg-[#050b14] text-white flex flex-col font-sans" dir="rtl">
        <header className="p-6 flex justify-between items-center border-b border-white/10 backdrop-blur-md sticky top-0 z-50">
            <div className="flex items-center gap-3">
                <img src="https://up6.cc/2026/02/177116640037762.png" alt="Logo" className="h-10" />
            ...
            {activeExam && (
                 <button onClick={() => setIsScanningMode(false)} className="bg-white/10 px-4 py-2 rounded-xl text-xs font-bold hover:bg-white/20 transition-all">إلغاء</button>
            )}
        </header>

        <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-10">
            <div className="text-center space-y-4 max-w-xs">
                <div className="w-24 h-24 bg-blue-600 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-2xl shadow-blue-500/20 border-4 border-white/5 animate-pulse">
                    <QrCode size={48} className="text-white" />
                </div>
                <h1 className="text-2xl font-black">{isScanningMode ? "تغيير اللجنة" : "بدء العمل"}</h1>
                <p className="text-slate-400 text-sm leading-relaxed">أهلاً بك أستاذ {currentUser?.name}. يرجى توجيه الكاميرا نحو باركود المظروف لفتح بيانات اللجنة.</p>
            </div>
  ...
  // --- الواجهة 2: سجل التحضير الشامل (بعد المسح) ---
  return (
    <div className="min-h-screen bg-slate-50 pb-36 font-sans text-right" dir="rtl">
      ...
      <div className="bg-[#050b14] text-white p-6 rounded-b-[2.5rem] shadow-2xl sticky top-0 z-30 border-b-4 border-blue-600">
        <div className="flex justify-between items-start mb-6">
            <div className="space-y-1">
                <div className="flex items-center gap-2 mb-2">
                    <span className="bg-blue-600 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-lg shadow-blue-900/50">اللجنة نشطة</span>
                    <button 
            ...
            <div className="bg-white/5 backdrop-blur-md p-4 rounded-3xl border border-white/10 text-center min-w-[90px] shadow-inner">
                ...
                <span className="text-[8px] text-slate-500 font-bold uppercase mt-1 block">نهاية الفترة</span>
            </div>
        </div>
        ...
        <div className="relative group">
            <input 
                ...
                className="w-full bg-white/10 border border-white/10 rounded-2xl py-4 px-12 text-sm focus:bg-white focus:text-slate-900 outline-none transition-all placeholder-white/30 font-bold"
            />
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 group-focus-within:text-blue-600" size={20} />
        </div>
      </div>

      {/* شريط الفلاتر الذكي */}
      <div className="flex gap-2 p-4 overflow-x-auto no-scrollbar sticky top-[195px] z-20 bg-slate-50/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
      ...
      {/* قائمة الطلاب الحصرية مرتبة حسب المراحل */}
      <div className="p-4 space-y-10">
        {Object.keys(sortedGroupedStudents).length === 0 ? (
                    ...
                    <div key={gradeTitle} className="space-y-4 animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {groupStudents.map(student => {
                                ...
                                return (
                                    <div key={student.id} className={`bg-white rounded-[2rem] p-5 shadow-sm border-2 transition-all flex items-center justify-between ${isPresent ? 'border-green-100 shadow-green-50/50' : isAbsent ? 'border-red-100 shadow-red-50/50' : 'border-slate-100 hover:border-blue-100'}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] text-blue-500 font-black uppercase">{student.className}</span>
                                                </div>
                                            </div>
                                        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-2xl border-t border-slate-200 z-40 pb-safe shadow-[0_-20px_40px_rgba(0,0,0,0.05)]">
          <button 
            onClick={() => {
                if(window.confirm('هل أنت متأكد من إنهاء الرصد وإرسال محضر الغياب النهائي؟')) {
          ...
          >
              <Send size={24} className="text-blue-400 animate-bounce-slow" />
